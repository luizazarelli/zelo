...
export class WorkerEntity {
	...
	addServiceType(serviceType: ServiceTypeEntity) {
		const hasServiceType = this._props.serviceTypes.some(
			(s) => s.props.id === serviceType.props.id,
		);
		if (hasServiceType) throw new WorkerServiceTypeAlreadyExists();

		this._props.serviceTypes.push(serviceType);
	}

	removeServiceType(serviceType: ServiceTypeEntity) {
		this._props.serviceTypes = this._props.serviceTypes.filter(
			(s) => s.props.id !== serviceType.props.id,
		);
	}
