...
import { serviceType, worker, workerServiceType } from "../schema";
...
export class WorkerMapper {
    static toDomain(
        workerModel: WorkerModelWithServiceType, 
    ): WorkerEntity { 
    ...
    static toPersistence(entity: WorkerEntity): InferInsertModel<typeof worker> & {  serviceTypes: ServiceTypeEntity[] } {
        const { userId, workingSince, createdAt, description, serviceTypes } = entity.props;
