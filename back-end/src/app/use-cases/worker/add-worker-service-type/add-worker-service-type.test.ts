...
import { AddWorkerServiceTypeUseCase } from "./add-worker-service-type.usecase";
...
describe("AddWorkerServiceType", () => {
	...
	it("Should fail if worker's not found", async () => {
		...
		vi.mocked(serviceTypeRepository.findById).mockResolvedValue(
			mockServiceTypeEntity(),
		);
